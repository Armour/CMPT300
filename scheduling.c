 ...
 *  that used to assign tasks to child processors in parent processor.
 *
 *  Project         : LyreBird
 *  Name            : Guo Chong
 ...
 *   used to assign tasks to child processors in parent processor.
 *
 *   Parameters:
 *      no parameters
 *
...
void round_robin(void) {
    get_time();                             /* Let processor of number cnt_rr decrypt this tweet */
    ...
    if (state == -1) {                      /* If select function failed */
        ...
        for (i = 0; i < processor_number_limit; ++i) {
            if (FD_ISSET(child_to_parent[i * 2], &rfds)) {
                int message;                            /* The message that read from child processor's pipe */
